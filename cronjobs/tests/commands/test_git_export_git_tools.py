...
def test_reset_repo_creates_local_branches(tmp_repo, mock_ls_remotes):
    repo = tmp_repo
    repo.branches.remote.create("origin/v1/buckets/main", repo.head.target)
    ...
    remote_ref = repo.lookup_reference("origin/v1/buckets/main")
    assert local_ref.target == remote_ref.target


def test_reset_repo_resets_local_branches_to_remote(tmp_repo, mock_ls_remotes):
    repo = tmp_repo
    repo.create_reference("refs/heads/v1/buckets/main", repo.head.target)
    ...
    local_ref = repo.lookup_reference("v1/buckets/main")
    remote_ref = repo.lookup_reference("origin/v1/buckets/main")
    assert local_ref.target != remote_ref.target
...
def test_reset_repo_deletes_extra_local_branches_and_tags(tmp_repo, mock_ls_remotes):
    repo = tmp_repo
    some_target = repo.references["refs/heads/main"].target
    repo.create_reference("origin/v1/buckets/main", some_target)
